...
function Cat(name, color){
	...
	var totalFurnitureArray = [];
	var totalCostArray = [];

	this.destroyFurniture = function(name, cost){
		totalFurnitureArray.push(name);
		totalCostArray.push(cost);
	}

	this.lastDestroyedFurniture = function(){
		var lastItem = {name: totalFurnitureArray[totalFurnitureArray.length-1], cost: totalCostArray[totalCostArray.length-1]};
		return lastItem;
	}
	...
	this.totalDestroyed = function(){
		...
		for(i = 0; i < totalCostArray.length; i++){
			totalCost += totalCostArray[i];
		}	
		return totalCost;
	}

	this.nthDestroyed = function(n){
		var nthFurniture = {name: totalFurnitureArray[n], cost: totalCostArray[n]};
		return nthFurniture;
	}
}
//end your code
...
myCat.destroyFurniture('Couch', 2500);
//end your code
